...
export function ActiveShiftPanel({ shift, breaks, workplaceName, roleName, now, busy = false, provisionalPay, expectedPay, salaryIncomplete, ...actions }: Props) {
  ...
  return <View style={styles.container}>
    <View style={[styles.hero, { backgroundColor: colors.surface, borderColor: metrics.isOnBreak ? colors.warning : colors.primary }]}>
      ...
      <Text accessibilityLabel={`${t('active.elapsed')} ${formatDurationCompact(metrics.elapsedMinutes)}`} style={[styles.timer, { color: colors.text }]}>{formatDurationCompact(metrics.elapsedMinutes)}</Text>
      <View style={styles.metrics}>
        ...
        {metrics.isOnBreak ? <Metric label={t('active.breakDuration')} value={formatDurationCompact(metrics.activeBreakMinutes)} /> : null}
      </View>
