...
describe('ActiveShiftPanel', () => {
  ...
  it('renders timestamp-derived Hebrew live metrics and accessible actions', () => {
    ...
    expect(screen.getAllByText('4:00 שעות').length).toBeGreaterThanOrEqual(1);
    fireEvent.press(screen.getByRole('button', { name: 'יציאה להפסקה ללא תשלום' }));
    expect(onStartBreak).toHaveBeenCalledWith(false);
    fireEvent.press(screen.getByRole('button', { name: 'יציאה להפסקה בתשלום' }));
    expect(onStartBreak).toHaveBeenCalledWith(true);
  });
  ...
  it('shows the running break and only offers returning from it', () => {
    ...
    expect(screen.getAllByText('0:08 שעות').length).toBeGreaterThanOrEqual(1);
    fireEvent.press(screen.getByRole('button', { name: 'חזרה מהפסקה' }));
    expect(onEndBreak).toHaveBeenCalledTimes(1);
    expect(screen.queryByRole('button', { name: 'יציאה להפסקה ללא תשלום' })).toBeNull();
  });
});
