...
describe('SQLite salary repositories', () => {
  it('persists profiles in minor units and resolves effective versions', async () => {
    ...
    const profile = createSalaryProfile({ baseHourlyRateMinor: 5850, defaultTravelReimbursementMinor: 1200, effectiveFrom: '2026-07-01' });
    await repository.create(profile);
    await expect(repository.resolveForDate('workplace-1', '2026-07-15')).resolves.toMatchObject({ baseHourlyRateMinor: 5850, effectiveFrom: '2026-07-01' });
    expect(database.runAsync.mock.calls[0]?.[0]).toContain('standard_hourly_rate_minor');
    expect(database.runAsync.mock.calls[0]?.slice(1)).toEqual(expect.arrayContaining([5850, 1200, '2026-07-01']));
  });
  ...
  it('creates a new effective rate version transactionally without overwriting history', async () => {
    ...
    const repository = new SqliteSalaryProfileRepository(database as unknown as SQLiteDatabase);
    const previous = createSalaryProfile({ id: 'old', effectiveFrom: '2026-01-01', effectiveTo: '2026-07-31', baseHourlyRateMinor: 5000 });
    const next = createSalaryProfile({ id: 'new', effectiveFrom: '2026-08-01', baseHourlyRateMinor: 6000 });
