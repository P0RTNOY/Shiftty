...
```bash
MAESTRO_CLI_NO_ANALYTICS=1 maestro test -e APP_ID=com.shifty.app .maestro/journeys
```

Run one journey while developing with the same `-e APP_ID=...` argument. The suite assumes the app is already installed and can launch without a disconnected development server.
...
- D is iOS-tagged because it confirms the native date picker. It saves a versioned profile with an eight-hour weekly threshold, creates deterministic eight-hour shifts on consecutive workweek dates, proves the first has no overtime segment, and verifies the later shift has exactly eight hours at 150% with no duplicate default-overtime segment. It also opens the assumptions disclosure.
...
- F completes a real finalized-zero shift, asserts the report's estimated total, generates PDF/CSV/calendar-only ICS through their native share sheets, and exports a backup through its native share sheet. It verifies the restore entry point but does not select and restore a file through the OS document picker.

Native share-sheet presentation is automated on iOS. File-content readback, document-picker restore, notification delivery, and locale/device appearance changes remain manual checks. Domain and integration tests remain authoritative for generated file contents, restore semantics, SQLite integrity, and numeric invariants; the Maestro flows verify the native actions and user-visible salary state transitions.
