...
Milestone 6 is based on `c2730d7cda7e08fd99631d2e91438862468f9222` on branch `codex/external-native-evidence-closure`. The complete native matrix ran at `f89d798fb68769a0b5a819fbf0d8c3467134b9fc`. Hosted CI then exposed an existing host-timezone-dependent fall-back overlap resolution, so `58ab0820cda2d8ff79eee886b87758d5000a8232` is the final product-bearing source candidate. The corrected Android Release was rebuilt and cold-launched; its seven-journey rerun is recorded in the detailed evidence. Physical-iPhone evidence remains tied to `f89d798` because the device was unavailable after the hosted correction. The final source SHA and successful hosted workflow run are recorded in the handoff after exact-SHA verification.

An isolated user-local Android toolchain now provides API 36, Build Tools 36.0.0, Platform Tools 37.0.1, Emulator 37.1.11, NDK 27.1.12297006, CMake 3.22.1, and an API 36 `google_apis` ARM64 system image. A clean arm64 Release built and installed as `com.shifty.app` on the pinned disposable emulator. The original full-matrix 45 MB APK has SHA-256 `8d9bfba9c341c2e5c0f7e54a69239cd016b5035ac17675ee060d7cedca89d143`; the rebuilt `58ab082` APK has SHA-256 `1cbbc1b9ce02edae7d11bf6a4f091a294022aa9097e053fbab5e6eae2578d37a`.
...
The current implementation run passed **138/138 Jest suites and 693/693 tests**. Focused timezone tests also pass with host `TZ` set to `Asia/Jerusalem`, `UTC`, `America/New_York`, and `Europe/London`. Expected SQLite constraint and foreign-key messages were emitted by passing negative integration tests. Hosted run `33068224029` at `9dccc7b216f35d04df0f9f311ed13154855f56f7` passed install, release configuration, typecheck, and lint, then exposed the timezone-dependent ambiguity. The resolver now selects the earlier valid target-zone instant independently of the host and preserves existing spring-gap normalization. The complete gate is rerun on the final documentation commit, after which only this branch is pushed and the exact-SHA hosted result is inspected.

No migration, backup V1 format, persisted salary total, historical snapshot, or public identifier changed. The timezone correction enforces the documented earlier-overlap occurrence for future resolution without adding an entitlement, premium, or automatic recalculation. The raw Expo check currently reports 13 exact SDK 57 patch notices; CI accepts only those reviewed pairs and fails on drift. The dependency boundary remains 16 transitive Expo/Metro developer-toolchain advisories (12 moderate, 4 high), with no compatible non-force repair; no forced dependency update was made.
